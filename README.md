...
もし自分が暗号を使う権利を正当化しなければならなくなったり、フランスみたいに個人が暗号を使うことを禁止されたら、
私はネットを使うのはやめようと思う。ネットで何かを発表したり、通信したり、意見を言ったり、書き込んだり。
その全てが権利の侵害になりうるものになるからだ。

そう、そしてネットはスマホだけの限定利用になる。
ネットの名時代に戻って、繋がないパソコンでプログラミングをするのだ。
その前に、ちょっとtor何かを使っていたので、もう既に警戒されているかも知れない。
そう、ここで私が思うのは「なぜ匿名が行けないのか？」という疑問だ。
自分の権利であり守らなければならないなら、私も自分なりのプライバシー論を披露しなければならないことになるだろう。
