...
暗号化の本体はビジュネル暗号だ。
もう一つ浴を言えば、エニグマのような確立性を導入したいが、これは今後の課題である。
秘密鍵暗号にも確率的な暗号化ができるように、使い捨てのIVをつけて、適切な暗号化モードで暗号化されるのが望ましい。
例えば置換をIVをシードに持つような確率的関数にすれば実現できると思われる。
ちなみに今の仕様では、鍵が固定のとき暗号化は確定的である。
しかし置換の生成がラウンドごとに動的なため、一見すると同じ文字でも異なる文字に変換されているように見える。
内部的に見れば、鍵のバイト列の位置が置換により変わっているだけであり確率的であるのとは違う。
確率的な関数には鍵に無関係な乱数パラメータが存在し、受診者がその計算結果を知る必要がない。
これを解決するために、PKPを使うことができる。
つまり暗号文を生成するのに符号の生成行列をかけて秘密鍵を使った符号語として変換し、
このように乱数化した暗号文はパリティ検査行列をかけて符号ベクトルをフィルターにかけて暗号文を抽出する。
この実装はこれからの課題である。
