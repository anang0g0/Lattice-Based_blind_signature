...
この変換は線形でないだけでなく、AESのようなアフィン変換でもない。
...
秘密鍵暗号にも確率的な暗号化ができるように、使い捨てのIVをつけて適切な暗号化モードで暗号化されるのが望ましい。
例えば置換をIVをシードに持つような確率的関数にすれば実現できると思われる。
ちなみに今の仕様では、鍵が固定のとき暗号化は確定的である。
...
このプログラムは汎用性を持たせるために計算結果だけを表示するようにしてあり、
ファイル暗号化など特定の目的を持ったアプリにはしないつもりである。

解読できるかな。

なんか今更って感じですが、符号理論は多項式としても考えられることを知る。
つまり既約多項式と平文多項式の積で符号語とし、既約多項式のあまりが0となればいい。
多項式としてみることができるという点で、ユークリッドアルゴリズムが存在するのだろう。
今までなんで多項式として扱う必要があるのかさっぱり見当がつかなかったけど、ユークリッドアルゴリスムにすでに答えがあったのだ。
そうなると多項式間にもう一度挑戦することになる。
巡回しない二次元符号とか。
